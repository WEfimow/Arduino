...
void setup() {
  ...
  crm.begin("RTP-MIDI Контроллер", interface, update, api, 115200);
...
void interface() {
  ...
  crm.output({OUTPUT_TABL, "r1", "Педаль 1", "(<c style='color:green'>OFF</c>)"});
  crm.output({OUTPUT_TABL, "r2", "Педаль 1", "(<c style='color:red'>ON</c>)"});
  crm.output({OUTPUT_TABL, "r3", "Педаль 1", "(<c style='color:red'>ON</c>)"});
  crm.output({OUTPUT_TABL, "r4", "Педаль 1", "(<c style='color:green'>OFF</c>)"});
  crm.output({OUTPUT_TABL, "r5", "Фейдер", "Экспрессия (<c style='color:green'>120</c>)"});
  crm.output({OUTPUT_HR, "1px", "-3px 10% 0"});
      
      String  icon = "&#xe808;"   ;  
      crm.input({INPUT_BUTTON, "b1", icon, "20px 30px 20px 30px", "r", "55"});
      crm.input({INPUT_BUTTON, "b1", "&#xe803;", "20px 30px 20px 30px", "r", "55"});
      crm.input({INPUT_BUTTON, "b1", "&#xe803;", "20px 30px 20px 30px", "r", "55"});
      crm.input({INPUT_BUTTON, "b1", "&#xe803;", "20px 30px 20px 30px", "r", "55"});

      crm.gauge({GAUDE_2, "G_2", "Фейдер (Экспрессия)", 0, 100, 35,
    {
