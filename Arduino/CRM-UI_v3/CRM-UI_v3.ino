...
void setup() {
  ...
  crm.contacts("my_mail@mail.ru", "1111111", "http://crims.noip.me:40080/myip");
...
void loop() {
  ...
  if (crm.btnSwStatus()) {
    ...
    crm.btnCallback("SLB", sleepTest);    // Check "SLB" SW button
  }
...
void tablt2() {
  ...
  crm.webUpdate("t2", m);
  crm.webUpdate("t1", m);
  crm.webNotif("i", "Заголовок 2: Millis " + m , 3, 1);
}
...
void interface() {
  ...
  // Пример:  &#xf1c9;
  String txt = F("&#xf1c9; Примером чтобы немалое если примером или - страдание а счастливой только стремящегося такие из-за лишь. &#xf1c9; Некое немалое возникает которое то немалое это восхваляющих, всю картину раскрою которое eсли упрекнуть - назвал никто не, ни тягостными.");
  ...
  crm.page("&#xe806; Датчики");
  crm.output({OUTPUT_HR, "1px", "20px 10% -31px"});
  crm.output({OUTPUT_TABL, "r1", "ПИД 1", "25.2°C (<c style='color:green'>OFF</c>)"});
  crm.output({OUTPUT_TABL, "r2", "ПИД 2", "29.8°C (<c style='color:red'>ON</c>)"});
  crm.output({OUTPUT_TABL, "r3", "ПИД 3", "26.4°C (<c style='color:red'>ON</c>)"});
  crm.output({OUTPUT_TABL, "r4", "ПИД 4", "15.4°C (<c style='color:green'>OFF</c>)"});
  crm.output({OUTPUT_TABL, "r5", "ПИД 5", "20.0°C (<c style='color:green'>OFF</c>)"});
  crm.output({OUTPUT_HR, "1px", "-3px 10% 0"});


  /*crm.page("Deep Sleep");
    // Поле ввода цифр
    // crm.input({[Тип], ["ID"], ["Заголовок / значок"], ["Значенгние по умолчанию"], ["паттерн, смотри в инете"]});
    crm.input({INPUT_NUMBER, "SLT", "Time sleep, sec", "60"});

    crm.input({INPUT_BUTTON, "SLB", "Sleep", "", "r", "25"});
    ...
    crm.selOpt({"240 MHz", "240"});
    crm.selOpt({"160 MHz", "160"});
    crm.selOpt({"80 MHz", "80"});
    crm.select({"cpu", "Frequency CPU", "240"});*/
  ...
  if (crm.var("select1").toInt()) {
    ...
    crm.group(G_START);
    crm.input({INPUT_TIME, "time2", "Начало"});
    crm.input({INPUT_TIME, "time3", "Конец"});
    crm.group(G_END);
  ...
  crm.input({INPUT_CHECKBOX, "chk1", "Кнопка перезагрузки", "0", "1"});
  crm.input({INPUT_CHECKBOX, "chk2", "Подсветка", "1"});

  // Выбор дней недели
  // crm.input({[Тип], ["ID"], ["Заголовок"], ["Значение по умолчанию"] });
  // Значение: строка элементов. Пример: "0010010"
  crm.input({INPUT_WEEK, "WD1", "Дни недели", "0011001"});
  ...
  crm.range({"range2", "Яркость", 52, 0, 84, 1, " ппг"});	//С единицами измерения
  crm.group(G_START);
  crm.range({"range3", "Громкость", 12, 0, 84, 1});            //Без единиц измерения
  crm.range({"range4", "Яркость", 52, 0, 84, 1, " ппг"});  //С единицами измерения
  crm.group(G_END);
